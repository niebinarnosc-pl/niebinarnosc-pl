...
The `outline` utility now sets `outline-width: 1px` by default to be more consistent with border and ring utilities. Furthermore all `outline-`<code class="inline">&lt;number&gt;</code> utilities default `outline-style` to `solid`, omitting the need to combine them with `outline`:
...
For example, the popular [Motion library for React](https://motion.dev/docs/react-quick-start) lets you animate to and from CSS variable values:
